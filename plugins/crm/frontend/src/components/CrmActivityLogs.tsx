...
export const CrmActivityLogs: React.FC = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await api.get<{ data: any }>('/crm/audit-logs?' + queryParams.toString());
      setLogs(res.data?.data || []);
      setTotalPages(res.data?.meta?.totalPages || 1);
    } catch (err) {
      console.error('Failed to fetch CRM audit logs', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getActionLabel = (action: string) => {
    return action.replace('crm.', '').replace('.', ' ').toUpperCase();
  };
  ...
  const getReadableDetails = (log: AuditLog) => {
    ...
    switch (log.action) {
      case 'crm.contact.created':
        return `Created contact "${details.name}" (${details.email})`;
      case 'crm.contact.updated':
        return `Updated contact "${details.name}" (Status: ${details.status})`;
      case 'crm.contact.deleted':
        return `Deleted contact "${details.name}"`;
      case 'crm.deal.created':
        return `Created deal "${details.title}" with value $${details.value.toFixed(2)}`;
      case 'crm.deal.updated':
        return `Updated deal "${details.title}" (Stage: ${details.stage}, Value: $${details.value.toFixed(2)})`;
      case 'crm.deal.deleted':
        return `Deleted deal opportunity "${details.title}"`;
      default:
  ...
  return (
    <div className="crm-activity-logs-container" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', marginTop: '1rem' }}>
        ...
        <table className="atlas-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={5} style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-tertiary)' }}>No CRM logs found.</td>
              </tr>
            ) : logs.map((log) => (
              <tr key={log.id}>
